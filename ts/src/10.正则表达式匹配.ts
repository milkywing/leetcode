...
/** 从左到右尝试模型 */
const isMatchCore = (s: string, p: string, index1: number, index2: number) => {
  ...
  if (matchFirst) {
    ...
    if (index2 + 1 < length2 && p[index2 + 1] === '*') {
      //
      return isMatchCore(s, p, index1, index2 + 2) || isMatchCore(s, p, index1 + 1, index2);
    }
    // 只是单纯的字母匹配，一起前进
