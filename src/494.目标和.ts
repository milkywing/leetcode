...
function findTargetSumWays(nums: number[], target: number): number {
  // 【特例优化】
  // 1. 如果 target 大于 sum，不可能实现，返回0
  // 2. target + sum 不是偶数，不能被二整除，返回0
 ...
 * 已知数组总和 sum = x + y ，目标表达式和 target = x - y，推出 x = (target + sum) / 2，
 * 转化为 01 背包问题：从数组中选出一批数，使其和为 (target + sum) / 2。可进一步转化为动态规划解法
 */
